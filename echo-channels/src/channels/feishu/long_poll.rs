...
impl WsClient {
        ...
        *self.running.lock().await = true;
        ...
        loop {
            ...
            if self.config.reconnect_count >= 0 {
                for i in 0..self.config.reconnect_count {
                    self.do_reconnect_delay(i).await;
                    if self.try_connect(handler.clone()).await {
                        break;
                    }
                }
                error!(
                ...
                let mut attempt = 0;
                loop {
                    self.do_reconnect_delay(attempt).await;
                    if self.try_connect(handler.clone()).await {
                        break;
                    }
                    attempt += 1;
    ...
    async fn try_connect(&mut self, handler: Arc<dyn MessageHandler>) -> bool {
        match self.connect(handler).await {
            Ok(()) => true,
            Err(e) => {
