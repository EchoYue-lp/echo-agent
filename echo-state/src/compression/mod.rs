...
/// Context generated by the framework is model-visible, not source evidence.
pub fn is_generated_context_message(message: &Message) -> bool {
    is_context_projection_message(message)
        || message.content.as_text_ref().is_some_and(|text| {
            text.starts_with("[Horizon compact:")
                || text.starts_with("[Used tools:")
                || text.starts_with(
            ...
            match horizon_compressor.compress(horizon_input).await {
                Ok(output) => {
                    ...
                    if evicted_count > 0
                        ...
                        && let Err(error) = promoter.promote(&output.evicted).await
                    {
...
mod tests {
    ...
    impl MemoryPromoter for FailingPromoter {
        fn promote(
            &self,
            _evicted: &[Message],
        ) -> futures::future::BoxFuture<'_, Result<MemoryPromotionReceipt>> {
            Box::pin(async {
                Err(
                    echo_core::error::MemoryError::TransientNoCommit("promotion failed".into())
                        .into(),
                )
            })
        }
    ...
    #[tokio::test]
    async fn horizon_promotion_failure_restores_original_messages() -> Result<()> {
        use echo_core::llm::types::{FunctionCall, ToolCall};

        let mut ctx = ContextManager::builder(100_000)
            .visibility_horizon(horizon::VisibilityHorizonConfig {
                active_window_turns: 1,
                ..Default::default()
            })
            .memory_promoter(Arc::new(FailingPromoter))
            .build();
        ctx.push(Message::system("system".into()));
        for index in 0..3 {
            ctx.push(Message::user(format!("question {index}")));
            let id = format!("call-{index}");
            ctx.push(Message::assistant_with_tools(vec![ToolCall {
                id: id.clone(),
            ...
            ctx.push(Message::assistant(format!("answer {index}")));
        }
        let original = ctx.messages().to_vec();
        assert!(ctx.prepare(None).await.is_err());
        let restored = serde_json::to_value(ctx.messages())
