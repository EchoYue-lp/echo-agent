...
impl LlmConfig {
    ...
    pub fn build_client(&self) -> Result<Box<dyn echo_core::llm::LlmClient>> {
        match self.provider {
            LlmProvider::OpenAi => {
                let client = crate::OpenAiClient::new(self.clone())?;
                Ok(Box::new(client))
            }
            LlmProvider::Anthropic => {
                let client = crate::AnthropicClient::with_base_url(
                    &self.base_url,
                    &self.api_key,
                    &self.model,
                );
            ...
            LlmProvider::Ollama => {
                let client = crate::OllamaClient::with_base_url(&self.base_url, &self.model);
                Ok(Box::new(client))
            }
        }
    }
