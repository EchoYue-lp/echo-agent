...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let config_path = {
        ...
        std::path::PathBuf::from(home).join(".echo-agent").join("config.yaml")
    };
    ...
    let llm_config = echo_integration::providers::config::LlmConfig::deepseek(auth_token, model_name);
    ...
    println!("  [{}] usage 非 None（A2 捕获 LlmUsage）: {:?}",
        if pass1 { "✓" } else { "✗" }, usage.is_some());
    all_pass &= pass1;

    if let Some(ref stats) = usage {
        let pass2 = stats.prompt_tokens + stats.completion_tokens > 0;
        println!("  [{}] tokens > 0（真实数据非假占位）: prompt={} completion={}",
            if pass2 { "✓" } else { "✗" }, stats.prompt_tokens, stats.completion_tokens);
        all_pass &= pass2;

        let pass3 = stats.model != "unknown";
        println!("  [{}] model 非 unknown: {}",
            if pass3 { "✓" } else { "✗" }, stats.model);
        all_pass &= pass3;

        let pass4 = stats.usage_reported;
        println!("  [{}] usage_reported == true: {}",
            if pass4 { "✓" } else { "✗" }, stats.usage_reported);
        all_pass &= pass4;

        let pass5 = stats.call_count >= 1;
        println!("  [{}] call_count >= 1: {}",
            if pass5 { "✓" } else { "✗" }, stats.call_count);
        all_pass &= pass5;
