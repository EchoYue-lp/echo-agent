...
scenarios:
  ...
  model-budget-tokenizer-policy:
    ...
    finding_refs: [finding.provider-capability-authority]
    evidence_refs: [evidence.provider-protocol-quality]
  ...
  structured-output:
    ...
    finding_refs: [finding.structured-output-main-path]
  ...
  changing-model-facts:
    ...
    next_step: audit 明确 framework 默认、provider 声明和应用 override 的 precedence 与刷新责任
---
...
核心 request/stream 已映射，三个当前问题进入 Finding，动态 model facts 保持 needs_review。
