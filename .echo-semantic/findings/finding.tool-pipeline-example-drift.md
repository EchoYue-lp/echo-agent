...
evidence_refs: [evidence.effects-extensions, evidence.workspace-structure, evidence.tool-pipeline-example-repair, evidence.tool-pipeline-example-verification]
...
候选修复从真实默认管线调用的结构化 tracing 读取阶段名称，动态展示顺序，并检查关键权限、守卫、执行和终态观察关系；不新增公开 runtime API。整合 `origin/main@93ee33d9` 后独立复审与本地完整门禁通过，Finding 置为 resolved。GitHub Issue #101 仍待 PR、远端 CI 与 main 交付后关闭。
