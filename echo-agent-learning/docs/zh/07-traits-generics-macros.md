...
```bash
cargo run -p echo_agent_learning --example chapter_07_traits_generics
```

代码位于 [`traits.rs`](../../src/traits.rs)。
...
实现。教学 crate 的
[`project_patterns.rs`](../../src/project_patterns.rs) 定义真实工具，并直接调用
生成的工具类型，不经过 LLM：

```bash
cargo run -p echo_agent_learning --example chapter_15_echo_agent_tool
```
