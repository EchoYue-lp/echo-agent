...
| 概念 | 它是什么 | 本项目示例 |
|------|----------|------------|
| package | 一个 `Cargo.toml` 描述的构建/发布单元 | `echo-agent-learning` |
| crate | 一次编译得到的库或二进制 | `echo_agent_learning` 库 crate |
...
package 名 `echo-agent-learning`，源码写 `use echo_agent_learning::...`。
...
```bash
# 快速做类型检查，不生成最终可执行文件
cargo check -p echo_agent_learning --all-targets

# 运行教学 crate 全部测试
cargo test -p echo_agent_learning

# 运行一个示例
cargo run -p echo_agent_learning --example chapter_01_basics

# 只运行名称中含 ownership 的测试
cargo test -p echo_agent_learning ownership

# 生成 API 文档
cargo doc -p echo_agent_learning --no-deps
...
[`echo-agent-learning/src/lib.rs`](../../src/lib.rs) 声明顶层模块：
...
`echo_agent_learning::smart_pointers::cow` 路径。
...
- [`echo-agent-learning/src/lib.rs`](../../src/lib.rs)：教学代码入口。
