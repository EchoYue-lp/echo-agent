...
///
/// ```
/// use echo_agent_learning::basics::unicode_preview;
...
```bash
cargo test --doc -p echo_agent_learning
cargo doc -p echo_agent_learning --no-deps --open
```
...
cargo test -p echo_agent_learning --all-targets

# 名字包含 ownership 的测试
cargo test -p echo_agent_learning ownership

# 只运行集成测试文件
cargo test -p echo_agent_learning --test learning_contract

# 需要查看测试打印时
cargo test -p echo_agent_learning test_name -- --nocapture
```
...
[`tests/documentation_contract.rs`](../../tests/documentation_contract.rs) 会递归检查本学习 crate 的 Markdown
