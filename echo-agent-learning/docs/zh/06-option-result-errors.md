...
```bash
cargo run -p echo_agent_learning --example chapter_06_errors
```
...
[`LearningError`](../../src/errors.rs) 使用 `thiserror` 生成 `Display` 和
...
```bash
cargo run -p echo_agent_learning --example chapter_06_errors
cargo test -p echo_agent_learning errors
```
