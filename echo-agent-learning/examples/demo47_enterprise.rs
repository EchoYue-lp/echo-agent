...
//! ```bash
//! # 基础运行
//! QWEN_API_KEY=your_key cargo run -p echo-agent-learning --example demo47_enterprise
//! ```
...
fn demo_skills_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("examples/demo_skills")
}
...
async fn demo_external_skills() -> Result<()> {
    ...
    let skills_dir = demo_skills_dir();
    ...
    let discovered = agent
        .discover_skills(&[DiscoveryScope::Custom(skills_dir)])
        .await?;
