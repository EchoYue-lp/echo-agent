...
```bash
cargo run -p echo_rust_learning --example chapter_09_arc_weak
```
...
教学代码的 [`AgentRegistry`](../../../echo-rust-learning/src/smart_pointers/arc_weak.rs) 只保存 Weak。
...
- [`arc_weak.rs`](../../../echo-rust-learning/src/smart_pointers/arc_weak.rs)：最小强弱引用注册表。
