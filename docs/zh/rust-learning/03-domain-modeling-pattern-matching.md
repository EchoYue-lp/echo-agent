...
```bash
cargo run -p echo_rust_learning --example chapter_03_domain_modeling
```

对应实现是 [`basics.rs`](../../../echo-rust-learning/src/basics.rs)。
...
[`unicode_preview`](../../../echo-rust-learning/src/basics.rs) 使用 `chars().take()`。`String` 不能用整数
