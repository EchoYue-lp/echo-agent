...
available for a cleanup retry; successful deactivation settles an active callback's debt, while
failed initialization/activation requires successful `unregister` cleanup. A failed callback is
not treated as withdrawn merely because the desired enabled set changed. See
