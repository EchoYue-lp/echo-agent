...
fn column_mean_std(df: &DataFrame, col_name: &str) -> Result<(f64, f64, usize)> {
    ...
    if !is_numeric(col.dtype()) {
        return Err(ToolError::InvalidParameter {
            ...
            message: format!("Column '{}' is not numeric (type: {})", col_name, col.dtype()),
        }.into());
    }
    let series = col.as_materialized_series();
    let n_valid = series.len() - series.null_count();
    Ok((series.mean().unwrap_or(0.0), series.std(0).unwrap_or(0.0), n_valid))
}
...
fn inverse_normal_cdf(p: f64) -> f64 {
    ...
    if p == 0.5 { return 0.0; }
    let a: [f64; 4] = [2.50662823884, -18.61500062529, 41.39119773534, -25.44106049637];
    let b: [f64; 4] = [-8.47351093090, 23.08336743743, -21.06224101826, 3.13082909312];
...
pub struct HypothesisTestTool {
    ...
    column1: String,
    #[tool_param(description = "Second column name (required for chi_square and correlation_significance)")]
    ...
    async fn run(&self, params: HypothesisTestToolParams) -> Result<ToolResult> {
        ...
        match params.test_type.as_str() {
            ...
            "correlation_significance" => correlation_significance(
                &df, &params.column1, params.column2.as_deref(), alpha,
            ),
            other => Err(ToolError::InvalidParameter {
                name: "test_type".to_string(),
                message: format!(
                    "Unsupported test type '{}'. Use: t_test, chi_square, correlation_significance", other
                ),
            }.into()),
        }
    }
...
fn t_test(df: &DataFrame, col1: &str, col2: Option<&str>, alpha: f64) -> Result<ToolResult> {
    ...
    if n1 < 2 || n2 < 2 {
        return Ok(ToolResult::success("Each column needs at least 2 non-null values for a t-test"));
    }
    ...
    let df_val = if denominator > 0.0 { numerator / denominator } else { n1 as f64 + n2 as f64 - 2.0 };
    let p_value = 2.0 * (1.0 - normal_cdf(t_stat.abs()));
    let significant = p_value < alpha;
    let conclusion = if significant {
        format!("Reject null hypothesis: means are significantly different (p={:.4} < α={:.2})", p_value, alpha)
    } else {
        format!("Fail to reject null: no significant difference in means (p={:.4} ≥ α={:.2})", p_value, alpha)
    };
...
fn chi_square_test(df: &DataFrame, col1: &str, col2: Option<&str>, alpha: f64) -> Result<ToolResult> {
    ...
    let c2 = df.column(col2_name).map_err(|e| ToolError::ExecutionFailed {
        tool: TOOL_NAME.to_string(), message: format!("Column '{}' not found: {}", col2_name, e),
    })?;
    let c1_str = c1.cast(&DataType::String).map_err(|e| ToolError::ExecutionFailed {
        tool: TOOL_NAME.to_string(), message: format!("Cast '{}' to string failed: {}", col1, e),
    })?;
    ...
    let col_totals: Vec<usize> = (0..c).map(|j| observed.iter().map(|row| row[j]).sum()).collect();
    ...
    for i in 0..r { for j in 0..c {
        ...
        expected[i][j] = exp;
        if exp > 0.0 { chi2_stat += ((observed[i][j] as f64 - exp).powi(2)) / exp; }
    }}
    let dof = (r - 1) * (c - 1);
    let p_value = chi_square_p_value(chi2_stat, dof);
    let significant = p_value < alpha;
    let conclusion = if significant {
        format!("Reject null: columns are dependent (p={:.4} < α={:.2})", p_value, alpha)
    } else {
        format!("Fail to reject null: columns appear independent (p={:.4} ≥ α={:.2})", p_value, alpha)
    };
    Ok(ToolResult::success_json(serde_json::json!({
...
fn correlation_significance(df: &DataFrame, col1: &str, col2: Option<&str>, alpha: f64) -> Result<ToolResult> {
    ...
    let mut cov = 0.0; let mut var1 = 0.0; let mut var2 = 0.0;
    for i in 0..v1.len() {
        if let (Some(a), Some(b)) = (v1[i], v2[i]) {
            let d1 = a - mean1; let d2 = b - mean2;
            cov += d1 * d2; var1 += d1 * d1; var2 += d2 * d2;
        }
    }
    let r = if var1 > 0.0 && var2 > 0.0 { cov / (var1.sqrt() * var2.sqrt()) } else { 0.0 };
    let denom = (1.0 - r * r).sqrt();
    let t_stat = if denom > 0.0 { r * ((n_valid as f64 - 2.0).sqrt()) / denom } else { if r.abs() >= 1.0 { f64::INFINITY } else { 0.0 } };
    let p_value = 2.0 * (1.0 - normal_cdf(t_stat.abs()));
    let significant = p_value < alpha;
    let conclusion = if significant {
        format!("Reject null: correlation is significant (r={:.4}, p={:.4} < α={:.2})", r, p_value, alpha)
    } else {
        format!("Fail to reject null: correlation not significant (r={:.4}, p={:.4} ≥ α={:.2})", r, p_value, alpha)
    };
    Ok(ToolResult::success_json(serde_json::json!({
...
impl ToolRunner<RegressionToolParams> for RegressionTool {
    async fn run(&self, params: RegressionToolParams) -> Result<ToolResult> {
        ...
        let path = security.validate_file(&params.data_path)?;
        let df = load_dataframe(&path, None)?;
        let features: Vec<&str> = params.feature_columns.split(',').map(|s| s.trim()).collect();
        if features.is_empty() {
            return Err(ToolError::InvalidParameter { name: "feature_columns".into(), message: "At least one feature column is required".into() }.into());
        }
        let target_col = df.column(&params.target_column).map_err(|e| ToolError::ExecutionFailed {
            tool: TOOL_NAME.to_string(), message: format!("Target column '{}' not found: {}", params.target_column, e),
        })?;
        ...
        for feat_name in &features {
            let feat_col = df.column(*feat_name).map_err(|e| ToolError::ExecutionFailed {
                tool: TOOL_NAME.to_string(), message: format!("Feature column '{}' not found: {}", feat_name, e),
            })?;
            ...
            let r = if var_x > 0.0 && total_ss > 0.0 { cov_xy / (var_x.sqrt() * total_ss.sqrt()) } else { 0.0 };
            let r_sq = r * r;
            let mut residual_var = 0.0; let mut pair_count = 0usize;
            for i in 0..x.len() { if let (Some(xi), Some(yi)) = (x[i], y[i]) { residual_var += (yi - (intercept + slope * xi)).powi(2); pair_count += 1; } }
            let se_slope = if pair_count > 2 && var_x > 0.0 { (residual_var / (pair_count as f64 - 2.0) / var_x).sqrt() } else { 0.0 };
            ...
            for i in 0..x.len() { if let Some(xi) = x[i] { predicted[i] += slope * (xi - mean_x); } }
        }
        let mut residual_ss = 0.0; let mut valid_count = 0usize;
        for i in 0..y.len() { if let Some(yi) = y[i] { residual_ss += (yi - predicted[i]).powi(2); valid_count += 1; } }
        let overall_r_squared = if total_ss > 0.0 { 1.0 - residual_ss / total_ss } else { 0.0 };
        ...
        if let Some(out_path) = &params.output_path {
            ...
            std::fs::write(&out, serde_json::to_string_pretty(&result).unwrap_or_default())
                .map_err(|e| ToolError::ExecutionFailed { tool: TOOL_NAME.to_string(), message: format!("Failed to write output: {}", e) })?;
        }
...
pub struct DescriptiveAdvancedTool {
    #[tool_param(description = "Absolute path to the data file")]
    data_path: String,
    #[tool_param(description = "Column names to analyze, comma-separated (default: all numeric columns)")]
    ...
    async fn run(&self, params: DescriptiveAdvancedToolParams) -> Result<ToolResult> {
        ...
        let target_cols: Vec<String> = if let Some(cols_str) = &params.columns {
            ...
            df.get_column_names().into_iter()
                .filter(|name| df.column(*name).map(|c| is_numeric(c.dtype())).unwrap_or(false))
                .map(|s| s.to_string()).collect()
        };
        if target_cols.is_empty() {
            return Ok(ToolResult::success("No numeric columns found for advanced descriptive statistics"));
        }
        ...
        for col_name in &target_cols {
            let col = df.column(col_name.as_str()).map_err(|e| ToolError::ExecutionFailed {
                tool: TOOL_NAME.to_string(), message: format!("Column '{}' not found: {}", col_name, e),
            })?;
            if !is_numeric(col.dtype()) {
            ...
            let std_dev = (values.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / (n as f64 - 1.0)).sqrt();
            let skewness = if std_dev > 0.0 { values.iter().map(|x| ((x - mean) / std_dev).powi(3)).sum::<f64>() / n as f64 } else { 0.0 };
            let kurtosis = if std_dev > 0.0 { values.iter().map(|x| ((x - mean) / std_dev).powi(4)).sum::<f64>() / n as f64 - 3.0 } else { 0.0 };
