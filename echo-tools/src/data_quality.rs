...
impl Tool for MissingValueAnalysisTool {
    fn name(&self) -> &str { "missing_value_analysis" }
    fn permissions(&self) -> Vec<ToolPermission> { vec![ToolPermission::Read] }
    ...
    fn execute(&self, parameters: ToolParameters) -> futures::future::BoxFuture<'_, Result<ToolResult>> {
        Box::pin(async move {
            let data_path = parameters.get("data_path").and_then(|v| v.as_str())
            ...
            let overall_pct = if row_count * df.width() > 0 { (total_missing as f64 / (row_count * df.width()) as f64) * 100.0 } else { 0.0 };
...
fn classify_missing_pattern(col: &Column, row_count: usize) -> &'static str {
    let null_count = col.null_count();
    if null_count == 0 { return "no_missing"; }
    if null_count == row_count { return "all_missing"; }
    ...
    if transitions <= 2 { "monotonic_missing" }
    else if transitions as f64 / (null_count as f64).max(1.0) > 0.5 { "random_missing" }
    else { "scattered_missing" }
}

fn suggest_imputation(dtype: &DataType, missing_pct: f64, pattern: &str) -> String {
    if pattern == "no_missing" { return "No imputation needed".to_string(); }
    if pattern == "all_missing" || missing_pct > 80.0 { return "Consider dropping column (>80% missing)".to_string(); }
    if is_numeric(dtype) {
        if missing_pct < 10.0 { "Mean/median imputation (low missing rate)".to_string() }
        else if missing_pct < 30.0 { "Median or interpolation imputation".to_string() }
        else { "Consider model-based imputation (high missing rate)".to_string() }
    } else if matches!(dtype, DataType::String | DataType::Categorical(_, _)) {
        if missing_pct < 10.0 { "Mode imputation or 'Unknown' category".to_string() }
        else { "Add 'Missing' category indicator".to_string() }
    } else if matches!(dtype, DataType::Datetime(_, _) | DataType::Date) {
        "Forward/backward fill or interpolation".to_string()
    } else { "Domain-specific imputation recommended".to_string() }
}
...
impl Tool for OutlierDetectionTool {
    ...
    fn execute(&self, parameters: ToolParameters) -> futures::future::BoxFuture<'_, Result<ToolResult>> {
        Box::pin(async move {
            let data_path = parameters.get("data_path").and_then(|v| v.as_str())
                .ok_or_else(|| ToolError::MissingParameter("data_path".to_string()))?;
            let method = parameters.get("method").and_then(|v| v.as_str()).unwrap_or("iqr");
            let default_threshold = if method == "iqr" { 1.5 } else { 3.0 };
            let threshold = parameters.get("threshold").and_then(|v| v.as_f64()).unwrap_or(default_threshold);
            let security = SecurityConfig::global();
            let path = security.validate_file(data_path)?;
            let df = load_dataframe(&path, None)?;
            let target_cols: Vec<String> = if let Some(cols_str) = parameters.get("columns").and_then(|v| v.as_str()) {
            ...
            Ok(ToolResult::success_json(serde_json::json!({ "file": data_path, "method": method, "threshold": threshold, "columns": columns_json })))
        })
    }
}
...
fn detect_zscore_outliers(values: &[f64], threshold: f64, col_name: &str) -> serde_json::Value {
    ...
    if std_dev == 0.0 { return serde_json::json!({ "name": col_name, "n_valid": n, "mean": mean, "std_dev": 0.0, "outlier_count": 0, "note": "Zero variance — no outliers possible" }); }
    let z_scores: Vec<f64> = values.iter().map(|x| (x - mean) / std_dev).collect();
    let outliers: Vec<f64> = values.iter().zip(z_scores.iter()).filter(|(_, z)| z.abs() > threshold).map(|(v, _)| *v).collect();
    let pct = if n > 0 { (outliers.len() as f64 / n as f64) * 100.0 } else { 0.0 };
    serde_json::json!({
    ...
    fn execute(&self, parameters: ToolParameters) -> futures::future::BoxFuture<'_, Result<ToolResult>> {
        Box::pin(async move {
            ...
            if let Some(rules_str) = parameters.get("rules").and_then(|v| v.as_str()) {
                let rules: Vec<serde_json::Value> = serde_json::from_str(rules_str)
                    .map_err(|e| ToolError::InvalidParameter { name: "rules".into(), message: format!("Invalid rules JSON: {}", e) })?;
                for rule in &rules {
                    let col_name = rule.get("column").and_then(|v| v.as_str()).ok_or_else(|| ToolError::InvalidParameter { name: "rules".into(), message: "Each rule must have a 'column' field".into() })?;
                    ...
                    match rule_type {
                        ...
                        other => { issues.push(serde_json::json!({ "column": col_name, "type": "unknown_rule", "detail": format!("Unknown rule type '{}' (supported: range, regex)", other), "severity": "low" })); }
                    }
                }
            }
            let severity_counts = { let mut counts = serde_json::Map::new(); for level in &["high", "medium", "low"] { counts.insert(level.to_string(), serde_json::json!(issues.iter().filter(|i| i.get("severity").and_then(|v| v.as_str()) == Some(level)).count())); } serde_json::Value::Object(counts) };
            Ok(ToolResult::success_json(serde_json::json!({ "file": data_path, "total_issues": issues.len(), "severity_counts": severity_counts, "issues": issues })))
        })
    }
}