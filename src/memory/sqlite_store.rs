...
//!
//! // 带向量检索（需要 Embedder）
//! use echo_agent::memory::{HttpEmbedder, Embedder};
...
impl SqliteStore {
    ...
    fn open(path: impl AsRef<Path>, embedder: Option<Arc<dyn Embedder>>) -> Result<Self> {
        ...
        let conn = Connection::open(&path)
            .map_err(|e| MemoryError::IoError(format!("打开 SQLite 数据库失败: {e}")))?;
        ...
        .map_err(|e| MemoryError::IoError(format!("SQLite PRAGMA 设置失败: {e}")))?;
    ...
    fn init_tables(conn: &Connection) -> Result<()> {
        ...
        .map_err(|e| MemoryError::IoError(format!("创建主表失败: {e}")))?;
        ...
        .map_err(|e| MemoryError::IoError(format!("创建 FTS5 索引失败: {e}")))?;
        ...
        .map_err(|e| MemoryError::IoError(format!("创建向量表失败: {e}")))?;
...
impl Store for SqliteStore {
    fn put<'a>(
        ...
        Box::pin(async move {
            ...
            .map_err(|e| MemoryError::IoError(format!("写入主表失败: {e}")))?;
            ...
            .map_err(|e| MemoryError::IoError(format!("删除 FTS 索引失败: {e}")))?;

            conn.execute(
                "INSERT INTO store_fts (namespace, key, content) VALUES (?1, ?2, ?3)",
                params![ns_key, key, search_text],
            )
            .map_err(|e| MemoryError::IoError(format!("写入 FTS 索引失败: {e}")))?;

            // 释放连接锁再做嵌入计算（嵌入是网络 IO，避免长时间持锁）
            drop(conn);
            ...
            if let Some(ref embedder) = self.embedder {
                match embedder.embed(&search_text).await {
                    Ok(vec) => {
                        ...
                        .map_err(|e| MemoryError::IoError(format!("写入向量表失败: {e}")))?;
    ...
    fn get<'a>(
        ...
        Box::pin(async move {
            ...
            match result {
                ...
                Err(e) => Err(MemoryError::IoError(format!("查询失败: {e}")).into()),
            }
        })
    }
        ...
        Box::pin(async move {
            ...
            let matched_keys: Vec<(String, f64)> = {
                let mut stmt = conn
                    ...
                    .map_err(|e| MemoryError::IoError(format!("FTS 查询准备失败: {e}")))?;

                stmt.query_map(params![ns_key, fts_query, limit as i64], |row| {
                    Ok((row.get::<_, String>(0)?, row.get::<_, f64>(1)?))
                })
                .map_err(|e| MemoryError::IoError(format!("FTS 查询失败: {e}")))?
                .filter_map(|r| r.ok())
                .collect()
            };
            ...
            if matched_keys.is_empty() {
                ...
                let mut stmt = conn
                    ...
                    .map_err(|e| MemoryError::IoError(format!("LIKE 查询准备失败: {e}")))?;

                let fallback_keys: Vec<String> = stmt
                    .query_map(params![ns_key, like_pattern, limit as i64], |row| {
                        row.get::<_, String>(0)
                    })
                    .map_err(|e| MemoryError::IoError(format!("LIKE 查询失败: {e}")))?
                    .filter_map(|r| r.ok())
                    .collect();
    ...
    fn delete<'a>(&'a self, namespace: &'a [&'a str], key: &'a str) -> BoxFuture<'a, Result<bool>> {
        Box::pin(async move {
            ...
            let affected = conn
                ...
                .map_err(|e| MemoryError::IoError(format!("删除主表失败: {e}")))?;
        ...
        Box::pin(async move {
            ...
            let scored = {
                ...
                let mut stmt = conn
                    ...
                    .map_err(|e| MemoryError::IoError(format!("查询向量表失败: {e}")))?;
                ...
                let mut scored: Vec<(f32, String)> = stmt
                    ...
                    .map_err(|e| MemoryError::IoError(format!("查询向量失败: {e}")))?
                    .filter_map(|r| r.ok())
                    .map(|(key, bytes)| {
