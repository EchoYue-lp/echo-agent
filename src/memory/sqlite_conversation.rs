...
impl SqliteConversationStore {
    ...
    fn init_tables(conn: &Connection) -> Result<()> {
        ...
        .map_err(|e| MemoryError::IoError(format!("建表失败: {e}")))?;
        Ok(())
    }
}
...
impl ConversationStore for SqliteConversationStore {
    ...
    fn get_conversation<'a>(
        ...
        Box::pin(async move {
            ...
            match result {
                ...
                Err(e) => Err(MemoryError::IoError(format!("查询对话失败: {e}")).into()),
            }
        })
    }
    ...
    fn list_conversations<'a>(
        ...
        Box::pin(async move {
            ...
            let mut stmt = conn
                .prepare(&sql)
                .map_err(|e| MemoryError::IoError(format!("准备查询失败: {e}")))?;
            ...
            let rows = stmt
                ...
                .map_err(|e| MemoryError::IoError(format!("查询对话列表失败: {e}")))?;
    ...
    fn update_conversation<'a>(
        ...
        Box::pin(async move {
            ...
            conn.execute(&sql, params_refs.as_slice())
                .map_err(|e| MemoryError::IoError(format!("更新对话失败: {e}")))?;
    ...
    fn delete_conversation<'a>(&'a self, conversation_id: &'a str) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            ...
            .map_err(|e| MemoryError::IoError(format!("删除对话失败: {e}")))?;
            Ok(())
        })
    }
    ...
    fn save_messages<'a>(
        ...
        Box::pin(async move {
            ...
            .map_err(|e| MemoryError::IoError(format!("清除旧消息失败: {e}")))?;
            ...
            for msg in messages {
                ...
                .map_err(|e| MemoryError::IoError(format!("插入消息失败: {e}")))?;
            }
            ...
            .map_err(|e| MemoryError::IoError(format!("更新对话时间戳失败: {e}")))?;
    ...
    fn get_messages<'a>(
        ...
        Box::pin(async move {
            ...
            let rows = stmt
                ...
                .map_err(|e| MemoryError::IoError(format!("查询消息失败: {e}")))?;
    ...
    fn count_messages<'a>(&'a self, conversation_id: &'a str) -> BoxFuture<'a, Result<usize>> {
        Box::pin(async move {
            ...
            let count: i64 = conn
                ...
                .map_err(|e| MemoryError::IoError(format!("统计消息失败: {e}")))?;
            Ok(count as usize)
        })
    }
}
