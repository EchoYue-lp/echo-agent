...
impl Default for TeamRunner {
    fn default() -> Self { Self { max_concurrent: 5, timeout_secs: 120 } }
}
...
impl TeamRunner {
    ...
    pub async fn fan_out(&self, team: &Team, task: &str) -> Vec<MemberResult> {
        ...
        if workers.is_empty() { return vec![]; }
        ...
        for worker in workers {
            ...
            handles.push(tokio::spawn(async move {
                ...
                match result {
                    Ok(Ok(output)) => MemberResult { name, role, output, success: true, duration_ms },
                    Ok(Err(e)) => MemberResult { name, role, output: format!("Error: {e}"), success: false, duration_ms },
                    Err(_) => MemberResult { name, role, output: format!("Timeout after {timeout}s"), success: false, duration_ms },
                }
            }));
        }
    ...
    pub async fn fan_out_to(&self, team: &Team, names: &[&str], task: &str) -> Vec<MemberResult> {
        ...
        for member in members {
            ...
            handles.push(tokio::spawn(async move {
                ...
                MemberResult { name, role, output, success, duration_ms: start.elapsed().as_millis() as u64 }
            }));
        }
