...
use super::SkillInfo;
use super::external::prompt_exec::{PromptContext, SkillSource, process_skill_content};
use super::external::types::{
...
///
/// Tracks both code-based skills (registered via [`Skill`](super::Skill) trait)
...
impl SkillRegistry {
    ...
    pub async fn activate(&mut self, name: &str) -> crate::error::Result<SkillContent> {
    ...
    /// 2. Substitutes variables (`${SKILL_DIR}`, `${SESSION_ID}`, `${ARGUMENTS}`, etc.)
    /// 3. Executes inline commands (`` !`cmd` `` and `` ```! cmd ``` ``)
    /// 4. Enumerates bundled resources
    pub async fn activate_with_args(
        &mut self,
        name: &str,
        args: &[String],
        source: SkillSource,
    ) -> crate::error::Result<SkillContent> {
        let descriptor = self.descriptors.get(name).ok_or_else(|| {
            crate::error::ReactError::Other(format!("Skill '{}' not found in catalog", name))
        })?;
        ...
        let raw_instructions = extract_body(&raw_content);
