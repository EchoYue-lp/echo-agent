...
pub trait PlanStore: Send + Sync {
    fn save_plan<'a>(
        &'a self,
        plan: &'a crate::agents::plan_execute::types::Plan,
    ) -> BoxFuture<'a, Result<()>>;
    fn load_plan<'a>(
        &'a self,
        plan_id: &'a str,
    ) -> BoxFuture<'a, Result<Option<crate::agents::plan_execute::types::Plan>>>;
...
impl SqlitePlanStore {
    ...
    fn plan_to_value(plan: &crate::agents::plan_execute::types::Plan) -> serde_json::Value {
        json!({
            "id": plan.id,
...
mod tests {
    use super::*;
    use crate::agents::plan_execute::types::{Plan, PlanStep};
