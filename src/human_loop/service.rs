...
//!
//! ```rust,ignore
//! use echo_agent::human_loop::{PermissionService, PermissionMode};
//! use std::sync::Arc;
//!
...
use crate::human_loop::approval_cache::SessionApprovalCache;
use crate::human_loop::audit::{PermissionAuditEntry, PermissionAuditSink};
use crate::human_loop::classifier::{Classifier, ClassifierContext, DenialTracker};
use crate::human_loop::permission::{
    PermissionRequest, PermissionRequestHandler, PermissionResponse, PermissionResponseDecision,
    PermissionUpdate, RiskLevel,
};
use crate::human_loop::policy::ApprovalScope;
use crate::human_loop::protected::{ProtectedPathChecker, ProtectedPathResult};
...
impl PermissionService {
    ...
    pub fn with_legacy_policy(
        self,
        _policy: Arc<dyn crate::tools::permission::PermissionPolicy>,
    ) -> Self {
    ...
    pub async fn check_with_permissions(
        ...
        // 3.5. 受保护路径检查（即使 Bypass 模式也受保护，但上面已经短路）
        ...
        // 5. DenialTracker 检查 — 连续拒绝过多则升级为人工审批
        {
    ...
    async fn check_with_handler(
        ...
        if matches!(response.decision, PermissionResponseDecision::Allowed) {
            let has_session_rule = response.rule_updates.iter().any(|u| {
                matches!(
                    u,
                    PermissionUpdate::AddRule {
                        source,
                        behavior,
                        ..
                    } if source == "session" && behavior == "allow"
                )
            });
            let scope = if has_session_rule {
