...
//!
//! ```rust,ignore
//! use echo_agent::human_loop::{PermissionRequest, PermissionRequestHandler, PermissionResponse};
//!
//! // 创建权限请求
    ...
    pub fn from_suggestion(suggestion: &Suggestion) -> Self {
        match &suggestion.action {
            ...
            SuggestedAction::AllowForSession => Self {
                decision: PermissionResponseDecision::Allowed,
                rule_updates: vec![PermissionUpdate::add_session_rule(suggestion.label.clone())],
                feedback: None,
                updated_input: None,
            },
            SuggestedAction::AllowAlways => Self {
                decision: PermissionResponseDecision::Allowed,
                rule_updates: vec![PermissionUpdate::add_permanent_rule(
            ...
            SuggestedAction::DenyAlways => Self {
                ...
                rule_updates: vec![PermissionUpdate::add_deny_rule(suggestion.label.clone())],
                feedback: None,
                updated_input: None,
            },
...
mod tests {
    ...
    fn test_permission_response_from_suggestion() {
        let s = Suggestion::allow_once();
        let resp = PermissionResponse::from_suggestion(&s);
        assert!(matches!(resp.decision, PermissionResponseDecision::Allowed));
