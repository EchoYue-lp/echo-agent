...
impl TaskStore for SqliteTaskStore {
    fn save_task<'a>(&'a self, task: &'a Task) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            let value = serde_json::to_value(task).map_err(|e| {
                crate::error::ReactError::Other(format!("save_task serialize: {}", e))
            })?;
            self.store
                .put(TASK_NAMESPACE, &task.id, value)
                .await
                .map_err(|e| crate::error::ReactError::Other(format!("save_task: {}", e)))
        })
    }
    ...
    fn load_task<'a>(&'a self, id: &'a str) -> BoxFuture<'a, Result<Option<Task>>> {
        Box::pin(async move {
            let item = self
                .store
                .get(TASK_NAMESPACE, id)
                .await
                .map_err(|e| crate::error::ReactError::Other(format!("load_task: {}", e)))?;

            match item {
                Some(item) => {
                    let task = serde_json::from_value(item.value).map_err(|e| {
                        crate::error::ReactError::Other(format!("load_task parse: {}", e))
                    })?;
                    Ok(Some(task))
                }
                None => Ok(None),
    ...
    fn load_all<'a>(&'a self) -> BoxFuture<'a, Result<Vec<Task>>> {
        Box::pin(async move {
            let results = self
                .store
                .search(TASK_NAMESPACE, "", 10000)
                .await
                .map_err(|e| crate::error::ReactError::Other(format!("load_all: {}", e)))?;
    ...
    fn delete_task<'a>(&'a self, id: &'a str) -> BoxFuture<'a, Result<bool>> {
        Box::pin(async move {
            self.store
                ...
                .await
                .map_err(|e| crate::error::ReactError::Other(format!("delete_task: {}", e)))
        })
    }
}
...
impl ExecutionCheckpoint {
    ...
    pub fn from_manager(plan_id: Option<String>, manager: &crate::tasks::TaskManager) -> Self {
    ...
    fn save_checkpoint<'a>(
        ...
        Box::pin(async move {
            let value = serde_json::to_value(checkpoint).map_err(|e| {
                crate::error::ReactError::Other(format!("save_checkpoint serialize: {}", e))
            })?;
            self.store
                .put(CHECKPOINT_NAMESPACE, &checkpoint.id, value)
                .await
                .map_err(|e| crate::error::ReactError::Other(format!("save_checkpoint: {}", e)))
        })
    }
    ...
    fn load_latest_checkpoint<'a>(
        ...
        Box::pin(async move {
            let results = self
                ...
                .await
                .map_err(|e| {
                    crate::error::ReactError::Other(format!("load_latest_checkpoint: {}", e))
                })?;
    ...
    fn list_checkpoints<'a>(
        ...
        Box::pin(async move {
            let results = self
                ...
                .await
                .map_err(|e| crate::error::ReactError::Other(format!("list_checkpoints: {}", e)))?;
