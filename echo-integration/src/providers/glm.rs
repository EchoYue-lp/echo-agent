//! GLM (Zhipu) provider — full `LlmClient` implementation for GLM-5.x.
//!
//! open.bigmodel.cn differences:
//! 1. GLM-5.x: `reasoning_effort` (max/xhigh/high/medium/low/minimal/none) +
//!    `thinking:{type:"enabled"|"disabled"}` combo toggle
//! 2. Auth: `Authorization: Bearer {api_key}`
//! 3. Base URL overridable via `GLM_BASE_URL` env var
...
const BASE_URL_ENV: &str = "GLM_BASE_URL";
...
impl LlmClient for GlmClient {
    fn chat(&self, request: ChatRequest) -> BoxFuture<'_, Result<ChatResponse>> {
        ...
        Box::pin(
            ...
            .instrument(info_span!("glm_chat", model = %model)),
        )
    }
    ...
    fn chat_stream(
        ...
        Box::pin(
            ...
            .instrument(info_span!("glm_chat_stream", model = %model)),
        )
    }
